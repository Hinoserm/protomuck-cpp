        ...
        switch (curr->type) {
            ...
            case PROG_FUNCTION:
                sprintf(buf, "%d: (line %d) FUNCTION: %s, VARS: %d, ARGS: %d",
                        i, curr->line, curr->data.mufproc->procname ? curr->data.mufproc->procname : "", curr->data.mufproc->vars, curr->data.mufproc->args);
                break;
            case PROG_LOCK:
            ...
            case PROG_FLOAT:
                ...
                break;
            case PROG_ADD:
                sprintf(buf, "%d: (line %d) ADDRESS: %d", i, curr->line, (int) (curr->data.addr->data - codestart));
                break;
            case PROG_TRY:
                sprintf(buf, "%d: (line %d) TRY: %d", i, curr->line, (int) (curr->data.call - codestart));
            case PROG_IF:
                sprintf(buf, "%d: (line %d) IF: %d", i, curr->line, (int) (curr->data.call - codestart));
                break;
            case PROG_JMP:
                sprintf(buf, "%d: (line %d) JMP: %d", i, curr->line, (int) (curr->data.call - codestart));
                break;
            case PROG_EXEC:
