...
mcp_frame_output_mesg(McpFrame *mfr, McpMesg *msg)
{
    ...
    if (mlineflag) {
        ...
        out += strlen(out);
#ifdef __CYGWIN__
        snprintf(datatag, sizeof(datatag), "%.8X", random() ^ random());
#else
        snprintf(datatag, sizeof(datatag), "%.8lX", random() ^ random());
#endif
        strcat(out, datatag);
    }
...
mcp_basic_handler(McpFrame *mfr, McpMesg *mesg, void *dummy)
{
    ...
    if (!*mesg->mesgname) {
        ...
        if (auth) {
            ...
            mcp_mesg_arg_append(&reply, "to", "2.1");
#ifdef __CYGWIN__
            snprintf(authval, sizeof(authval), "%.8X", random() ^ random());
#else
	    snprintf(authval, sizeof(authval), "%.8lX", random() ^ random());
#endif
