...
int add_staticvar(COMPSTATE *, const char *, int valtype);
...
#define ADDRLIST_ALLOC_CHUNK_SIZE 4096
...
get_address(COMPSTATE *cstat, struct INTERMEDIATE *dest, int offset)
{
    ...
    if (cstat->addrcount >= cstat->addrmax) {
        ...
        log_status("COMPILE: Expanding address pool to %d.", cstat->addrmax);
    }
...
next_word(COMPSTATE *cstat, const char *token)
{
    ...
    else if (staticvar(cstat, token))
        new_word = stvar_word(cstat, token);
...
struct INTERMEDIATE *
stvar_word(COMPSTATE *cstat, const char *token)
{
    struct INTERMEDIATE *nw;
    int i, var_no;
    ...
    nw = new_inst(cstat);
    nw->no = cstat->nowords++;
    nw->in.type = PROG_STVAR;
    nw->in.line = cstat->lineno;
    for (var_no = i = 0; i < MAX_VAR; i++) {
        if (!cstat->staticvars[i])
            break;
        if (!string_compare(token, cstat->staticvars[i]))
            var_no = i;
    }
    nw->in.data.number = var_no;
...
int
staticvar(COMPSTATE *cstat, const char *token)
{
    int i;
