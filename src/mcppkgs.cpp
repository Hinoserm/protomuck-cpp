...
void
show_mcp_info(McpFrame *mfr, char *topic, char *text)
{
    McpMesg msg;
    McpVer supp = mcp_frame_package_supported(mfr, "org-fuzzball-notify");

    if (supp.verminor != 0 || supp.vermajor != 0) {
        mcp_mesg_init(&msg, "org-fuzzball-notify", "info");
        mcp_mesg_arg_append(&msg, "text", text);
        mcp_mesg_arg_append(&msg, "topic", topic);
        mcp_frame_output_mesg(mfr, &msg);
        mcp_mesg_clear(&msg);
    ...
    if (!string_compare(msg->mesgname, "set")) {
        ...
        *ptr = '\0';
        ...
        if (!string_compare(category, "prop")) {
            ...
            const char *pName = unparse_object(player, obj); 
            log_status("PROGRAM SAVED: %s by %s(%d)\n",
                       pName, NAME(player), player);
	    ...
	    snprintf(buf, BUFFER_LEN, "Compilation of %s complete.", pName);
            show_mcp_info(mfr, "simpleedit-set", buf);
