...
muf_mcp_callback(McpFrame *mfr, McpMesg *mesg, McpVer version, void *context)
{
	...
	inst *tmpoper;
        ...
        tmpfr = interp(descr, user, getloc(user), obj, -1, PREEMPT,
                       STD_REGUID, 0);
        if (tmpfr) {
            tmpoper = tmpfr->argument.st + --tmpfr->argument.top;
            CLEAR(tmpoper);
    ...
    if (destfr) {
        ...
        for (arg = mesg->args; arg; arg = arg->next) {
            if (!arg->value) {
                ...
                argval.type = PROG_ARRAY;
                argval.data.array =
                    new_array_packed(mcp_mesg_arg_linecount(mesg, arg->name), 0);
...
prim_mcp_register(PRIM_PROTOTYPE)
{
    ...
    if (!oper[2].data.string || !*oper[2].data.string->data)
        abort_interp("Package name cannot be a null string. (1)");

    pkgname = oper[2].data.string->data;
    vermin.vermajor = (int) oper[1].data.fnumber;
    vermin.verminor = (int) (oper[1].data.fnumber * 1000) % 1000;
    vermax.vermajor = (int) oper[0].data.fnumber;
    vermax.verminor = (int) (oper[0].data.fnumber * 1000) % 1000;
...
prim_mcp_supports(PRIM_PROTOTYPE)
{
    ...
    pkgname = oper[0].data.string->data;
...
prim_mcp_bind(PRIM_PROTOTYPE)
{
    ...
    if (oper[0].type != PROG_ADD)
        abort_interp("Function address expected. (3)");
    if (!oper[2].data.string || !*oper[2].data.string->data)
        abort_interp("Package name cannot be a null string. (1)");
    if (!OkObj(oper[0].data.addr->progref)
        || (Typeof(oper[0].data.addr->progref) != TYPE_PROGRAM)) {
        abort_interp("Invalid address. (3)");
    }
    if (program != oper[0].data.addr->progref) {
        abort_interp("Destination address outside current program. (3)");
    }

    pkgname = oper[2].data.string->data;
    msgname = oper[1].data.string ? oper[1].data.string->data : pad_char;
    ...
    ptr->addr = oper[0].data.addr->data;
}
...
fbgui_muf_event_cb(GUI_EVENT_CB_ARGS)
{
    ...
    while (name) {
        ...
        temp2.data.array = new_array_packed(lines, 0);
...
prim_gui_available(PRIM_PROTOTYPE)
{
    ...
    ver = GuiVersion(oper[0].data.number);
    fver = ver.vermajor + (ver.verminor / 1000.0);
    PushFloat(fver);
}
...
prim_gui_dlog_create(PRIM_PROTOTYPE)
{
    ...
    /* descr  type   title  args */

    if (mlev < tp_mcp_muf_mlev)
        abort_interp(tp_noperm_mesg);
    if (oper[3].type != PROG_INTEGER)
        abort_interp("Integer descriptor number expected. (1)");
    if (!GuiSupported(oper[3].data.number))
        abort_interp
    ...
    title = oper[1].data.string ? oper[1].data.string->data : pad_char;
    wintype = oper[2].data.string ? oper[2].data.string->data : pad_char2;
    arr = oper[0].data.array;
    mfr = descr_mcpframe(oper[3].data.number);

    if (!mfr)
        abort_interp("Invalid descriptor number. (1)");

    dlogid = gui_dlog_alloc(oper[3].data.number, fbgui_muf_event_cb,
...
prim_gui_dlog_show(PRIM_PROTOTYPE)
{
    ...
    if (!oper[0].data.string || !oper[0].data.string->data[0])
        abort_interp("Invalid dialog ID.");

    result = GuiShow(oper[0].data.string->data);
...
prim_gui_dlog_close(PRIM_PROTOTYPE)
{
    ...
    result = GuiClose(oper[0].data.string->data);
    if (result == EGUINOSUPPORT)
        abort_interp("Internal error: GUI not available.");
    if (result == EGUINODLOG)
        abort_interp("Invalid dialog ID.");

    muf_dlog_remove(fr, oper[0].data.string->data);
}
...
prim_gui_ctrl_create(PRIM_PROTOTYPE)
{
    ...
    dlogid = oper[3].data.string->data;
    ctrltype = oper[2].data.string->data;
    ctrlid = oper[1].data.string ? oper[1].data.string->data : NULL;
    arr = oper[0].data.array;
...
prim_gui_ctrl_command(PRIM_PROTOTYPE)
{
    ...
    /* oper1  oper2  oper3   oper4 */
    /* dlogid ctrlid command args */

    if (mlev < tp_mcp_muf_mlev)
        abort_interp(tp_noperm_mesg);
    if (oper[3].type != PROG_STRING)
        abort_interp("Dialog ID string expected. (1)");
    if (!oper[3].data.string || !*oper[3].data.string->data)
    ...
    if (!oper[1].data.string || !*oper[1].data.string->data)
        abort_interp("Non-null control command string expected. (2)");
    if (oper[0].type != PROG_ARRAY)
        abort_interp("Dictionary of arguments expected. (4)");

    dlogid = oper[3].data.string->data;
    ctrlid = oper[2].data.string ? oper[2].data.string->data : NULL;
    ctrlcmd = oper[1].data.string->data;
    arr = oper[0].data.array;
...
prim_gui_value_set(PRIM_PROTOTYPE)
{
    ...
    if (mlev < tp_mcp_muf_mlev)
        abort_interp(tp_noperm_mesg);
    if (oper[0].type != PROG_STRING && oper[0].type != PROG_ARRAY)
        abort_interp("String or string list control value expected. (3)");

    if (oper[1].type != PROG_STRING)
        abort_interp("String control ID expected. (2)");
    if (!oper[1].data.string || !oper[1].data.string->data[0])
        abort_interp("Invalid control ID. (2)");
    ...
    dlogid = oper[2].data.string->data;
    ...
    if (oper[0].type == PROG_STRING) {
        ...
        value = oper[0].data.string ? oper[0].data.string->data : pad_char;
        valarray[0] = (char *) malloc(sizeof(char) * strlen(value) + 1);

        strcpy(valarray[0], value);
    } else {
        count = array_count(oper[0].data.array);
        valarray = (char **) malloc(sizeof(char *) * count);
        ...
        for (i = 0; i < count; i++) {
            ...
            temp2 = array_getitem(oper[0].data.array, &temp1);
            if (!temp2) {
                break;
            }
            switch (temp2->type) {
...
prim_gui_values_get(PRIM_PROTOTYPE)
{
    ...
    dlogid = oper[0].data.string->data;
    nw = new_array_dictionary();
    name = GuiValueFirst(oper[0].data.string->data);
    while (name) {
        int i;
...
prim_gui_value_get(PRIM_PROTOTYPE)
{
    ...
    if (oper[1].type != PROG_STRING)
        abort_interp("String dialog ID expected. (1)");
    if (!oper[1].data.string || !*oper[0].data.string->data)
        abort_interp("Non-null string dialog ID expected. (1)");

    if (oper[0].type != PROG_STRING)
        abort_interp("String control ID expected. (2)");
    if (!oper[0].data.string || !*oper[1].data.string->data)
        abort_interp("Non-null string control ID expected. (2)");

    dlogid = oper[1].data.string->data;
    ctrlid = oper[0].data.string->data;

    lines = gui_value_linecount(dlogid, ctrlid);
    nw = new_array_packed(lines, 0);
