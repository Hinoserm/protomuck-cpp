...
void
__prim_nuku_rp_getstatus_1(PRIM_PROTOTYPE)
{
...
void
__prim_funcprof_array(PRIM_PROTOTYPE)
{
    int i;
    stk_array *nw;
    struct inst temp1;
...
__prim_notify_descriptor_nocr(PRIM_PROTOTYPE)
{
    ...
    if (oper1->data.string) {
        strcpy(buf, oper1->data.string->data.c_str());
...
void
__prim_test(PRIM_PROTOTYPE)
{
    log_status("THE CAKE IS: %s\r\n", mthis->info->name);
}

void
__prim_mod_runprim(PRIM_PROTOTYPE)
{
...
static struct mod_primlist prims[] = {
    ...
    {"__prim_notify_descriptor_nocr", "NOTIFY_DESCRIPTOR_NOCR", NULL, 0, NULL,
     0},
...
struct mod_info *
__get_module_info(struct module *m)
{
    mthis = m;
    return &minfo;
}
