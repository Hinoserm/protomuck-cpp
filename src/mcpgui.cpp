...
gui_dlog_alloc(int descr, Gui_CB callback, GuiErr_CB error_cb, void *context)
{
    ...
    while (1) {
#ifdef __CYGWIN__
        snprintf(tmpid, sizeof(tmpid), "%.08X", random() ^ random());
#else
        snprintf(tmpid, sizeof(tmpid), "%.08lX", random() ^ random());
#endif
...
GuiEdit(const char *dlogid, const char *pane, const char *id, const char *text,
        const char *value, int width, int readonly, int layout)
{
    char buf[32];

    snprintf(buf, sizeof(buf), "%d", width);
    return gui_ctrl_make_l(dlogid, "edit", pane, id, text, value, layout,
                           "width", buf, "readonly", readonly ? "1" : "0", NULL);
}
...
GuiMulti(const char *dlogid, const char *pane, const char *id,
         const char *value, int width, int height, int fixed, int readonly, int layout)
{
    char widthbuf[32];
    char heightbuf[32];
