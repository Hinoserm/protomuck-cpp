...
#define ARRAY_PRESIZE_CHUNK 128
...
typedef struct stk_array_t {
	...
	int items;                  /* number of items in array */
        int presize;                /* number of pre-allocated items available */
...
int array_set_strkey_arrval(stk_array ** arr, const char *key, stk_array *arr2); 
int array_set_strkey_intval(stk_array ** arr, const char *key, MUFINT val);
...
int array_compare(stk_array *arr1, stk_array *arr2);
