...
prim_nbsockrecv_char(PRIM_PROTOTYPE)
{
    ...
    //mystring = (char *) malloc(3); // ???? --hinoserm, june 23 2014
...
prim_nbsockopen(PRIM_PROTOTYPE)
{
    ...
    if (!(myhost = gethostbyname(oper[1].data.string->data))) {
        ...
        bcopy((char *) myhost->h_addr, (char *) &name.sin_addr, myhost->h_length);
...
prim_nbsock6open(PRIM_PROTOTYPE)
{
    ...
    if (!(myhost = gethostbyname2(oper[1].data.string->data, AF_INET6))) {
        ...
        bcopy((char *) myhost->h_addr, (char *) &name.sin6_addr, myhost->h_length);
