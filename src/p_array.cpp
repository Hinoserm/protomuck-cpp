...
/* integer - stackrange count */
    ...
    if (result > 0) {
        ...
        new_union = new_array_packed(0, 0);
    }
...
/* arr  Array   */
    ...
    nu = new_array_packed(count, 0);
    tmparr = new inst *[count];
        ...
        if (prop_read_perms(ProgUID, ref, buf, mlev)) {
            ...
            if (prptr) {
                ...
                switch (PropType(prptr)) {
                    case PROP_STRTYP:
                        temp2.type = PROG_STRING;
                        temp2.data.string = alloc_prog_string(PropDataUNCStr(prptr));
                        break;
                    case PROP_LOKTYP:
...
prim_array_put_proplist(PRIM_PROTOTYPE)
{
    ...
    *fmtout++ = '\0';
    ...
    char cntrpname[BUFFER_LEN];

    strcpy(cntrpname, propname);
        ...
        do {
            ...
            *fmtout++ = '\0';
            ...
            switch (oper4->type) {
                ...
                case PROG_INTEGER:
                    pdat.flags = PROP_INTTYP;
                    isempty = !(pdat.data.val = oper4->data.number);
                    break;
                case PROG_FLOAT:
                    pdat.flags = PROP_FLTTYP;
                    isempty = !(pdat.data.fval = oper4->data.fnumber);
                    break;
                case PROG_OBJECT:
                    pdat.flags = PROP_REFTYP;
                    isempty = ((pdat.data.ref = oper4->data.objref) == -1);
                    break;
                case PROG_LOCK:
                    pdat.flags = PROP_LOKTYP;
                    isempty = !(pdat.data.lok = copy_bool(oper4->data.lock));
                    break;
            }
            if (!isempty) {
                set_property(ref, propname, &pdat);
    ...
    if (count) {
        if (tp_proplist_int_counter) {
            ...
            pdat.flags = PROP_INTTYP;
            pdat.data.val = count;
            set_property(ref, cntrpname, &pdat);
        } else {
            sprintf(buf, "%d", count);
            pdat.flags = PROP_STRTYP;
            pdat.data.str = buf;
...
prim_array_findval(PRIM_PROTOTYPE)
{
    ...
    arr = oper[1].data.array;
    nw = new_array_packed(0, (array_count(arr) + 1) / 3);
    if (array_first(arr, &temp1)) {
        do {
    ...
    while (!done) {
        ...
        switch (in->type) {
            ...
            case PROG_FLOAT:
                ...
                break;
            case PROG_LOCK:
                text = unparse_boolexp(buf, ProgUID, in->data.lock, 1);
                break;
            default:
                text = "<UNSUPPORTED>";
                break;
    ...
    if (array_first(arr, &temp1)) {
        do {
            if (temp1.type == PROG_STRING) {
                if (equalstr((char *) DoNullInd(oper[0].data.string), (char *) DoNullInd(temp1.data.string))) {
                    in = array_getitem(arr, &temp1);
    ...
    if (array_first(arr, &temp1)) {
        do {
            ...
            if (in->type == PROG_STRING) {
                if (equalstr((char *) DoNullInd(oper[0].data.string), (char *) DoNullInd(in->data.string))) {
                    array_setitem(&nw, &temp1, in);
                }
...
prim_array_cut(PRIM_PROTOTYPE)
{
    ...
    if (!nu1)
        nu1 = new_array_packed(0, 0);
    if (!nu2)
        nu2 = new_array_packed(0, 0);
...
void
array_props_wildcard(stk_array *arr, dbref player, dbref thing, const char *dir, const char *wild, int mlev)
{
    char propname[BUFFER_LEN];
    char wld[BUFFER_LEN];
    char buf[BUFFER_LEN];
            ...
            if (prop_read_perms(player, thing, buf, mlev)) {
                if (!*ptr || recurse) {
                    ...
                    if (prptr) {
                        ...
                        if (goodflag) {
                            array_set_strkey(&arr, buf, &temp2);
                        }
                    }
                }
                if (recurse)
                    ptr = (char *) "**";
                array_props_wildcard(arr, player, thing, buf, ptr, mlev);
            }
        }
        propadr = next_prop(pptr, propadr, propname);
    }
...
void
prim_properties_array(PRIM_PROTOTYPE)
{
    stk_array *arr;
    ...
    array_props_wildcard(arr, ProgUID, oper[1].data.objref, "", DoNullInd(oper[0].data.string), mlev);
