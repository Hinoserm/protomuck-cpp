...
# Newer compilers don't like what we do to time_t and warns about it, so we
# use no-strict-aliasing because the fix is uglier.
#   -Alynna 12-25-2006.
# 
# Those time_t warnings are actually pretty important in catching some bugs
# related to 32<->64bit platform issues.  I've corrected most of the
# warnings so we should be safe to remove this now.
#   -Hinoserm 08-12-2010

OPTIM=-Wall -g -O2 -U_FORTIFY_SOURCE
...
CXX=@CXX@
DEFS=-DHAVE_CONFIG_H
LIBR=-lz -rdynamic -ldl -lmysqlclient -lssl -lcrypto -lpcre  -L/usr/lib -lpcre -lm -lnsl
...
# Include path, Cflags...
#
CFLAGS=${OPTIM} ${DEFS} -I${INCLUDE}  -I/usr/include -Imysql -L/usr/lib/mysql 
CXXFLAGS=${OPTIM} ${DEFS} -I${INCLUDE}  -I/usr/include -Imysql -L/usr/lib/mysql 
...
CSRC= array.c boolexp.c cgi.c compile.c compress.c create.c db.c  \
	...
	p_connects.c p_db.c p_error.c p_float.c p_file.c p_html.c p_http.c \
        ...
        p_stack.c p_strings.c p_system.c p_regex.c \
      ...
      sanity.c newhttp.c version.c sha1.c
